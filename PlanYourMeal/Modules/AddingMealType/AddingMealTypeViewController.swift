...
class AddingMealTypeViewController: UIViewController {
    ...
    @IBAction func addTapped(_ sender: UIButton) {
        if caloriesTextField.text != "", mealTypeNameTextField.text != "" {
            delegate?.getInfoForNewMeal(image: mealTypeImage.image ?? #imageLiteral(resourceName: "Lunch"), calories: caloriesTextField.text ?? "0", name: mealTypeNameTextField.text ?? "New meal")
